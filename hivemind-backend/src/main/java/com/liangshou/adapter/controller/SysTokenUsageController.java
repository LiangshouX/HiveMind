 ...
 * SysTokenUsage 控制器
 * <p>
 * 提供 SysTokenUsage 相关的 RESTful API 接口
 */
@Tag(name = "SysTokenUsage API", description = "SysTokenUsage 管理相关接口")
@RestController
@RequestMapping("/api/sys-token-usages")
@RequiredArgsConstructor
public class SysTokenUsageController {

    private final ISysTokenUsageService service;

    /**
     * 根据主键查询 SysTokenUsage 详情
     *
     * @param id 主键ID
     * @return 包含 SysTokenUsageVO 详情的 Result 对象
     */
    @Operation(summary = "查询详情", description = "根据主键获取单条记录")
    @GetMapping("/{id}")
    public Result<SysTokenUsageVO> getById(@PathVariable Long id) {
    ...
    @GetMapping
    public Result<PageResult<SysTokenUsageVO>> page(
     ...
     * 新增 SysTokenUsage 记录
     *
     * @param dto 包含新增数据的 SysTokenUsageDTO 对象
     * @return 包含保存结果(true/false)的 Result 对象
     */
    @Operation(summary = "新增记录", description = "创建一条新的数据记录")
    @PostMapping
    public Result<Boolean> save(@RequestBody SysTokenUsageDTO dto) {
        String userId = SecurityUtils.getCurrentUserId();
        return Result.success(service.save(userId, dto));
    }

    /**
     * 根据主键更新 SysTokenUsage 记录
     *
     * @param id  主键ID
     * @param dto 包含更新数据的 SysTokenUsageDTO 对象
     * @return 包含更新结果(true/false)的 Result 对象
     */
    @Operation(summary = "更新记录", description = "根据主键更新已有记录")
    @PutMapping("/{id}")
    public Result<Boolean> update(@PathVariable Long id, @RequestBody SysTokenUsageDTO dto) {
        String userId = SecurityUtils.getCurrentUserId();
        dto.setId(id);
