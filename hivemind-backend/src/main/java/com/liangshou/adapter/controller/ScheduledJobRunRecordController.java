...
/**
 * ScheduledJobRunRecord 控制器
 * <p>
 * 提供 ScheduledJobRunRecord 相关的 RESTful API 接口
 */
@Tag(name = "ScheduledJobRunRecord API", description = "ScheduledJobRunRecord 管理相关接口")
@RestController
@RequestMapping("/api/scheduled-job-run-records")
@RequiredArgsConstructor
public class ScheduledJobRunRecordController {

    private final IScheduledJobRunRecordService service;

    /**
     * 根据主键查询 ScheduledJobRunRecord 详情
     *
     * @param id 主键ID
     * @return 包含 ScheduledJobRunRecordVO 详情的 Result 对象
     */
    @Operation(summary = "查询详情", description = "根据主键获取单条记录")
    @GetMapping("/{id}")
    public Result<ScheduledJobRunRecordVO> getById(@PathVariable Long id) {
    ...
    @GetMapping
    public Result<PageResult<ScheduledJobRunRecordVO>> page(
    ...
    /**
     * 新增 ScheduledJobRunRecord 记录
     *
     * @param dto 包含新增数据的 ScheduledJobRunRecordDTO 对象
     * @return 包含保存结果(true/false)的 Result 对象
     */
    @Operation(summary = "新增记录", description = "创建一条新的数据记录")
    @PostMapping
    public Result<Boolean> save(@RequestBody ScheduledJobRunRecordDTO dto) {
        String userId = SecurityUtils.getCurrentUserId();
        return Result.success(service.save(userId, dto));
    }

    /**
     * 根据主键更新 ScheduledJobRunRecord 记录
     *
     * @param id  主键ID
     * @param dto 包含更新数据的 ScheduledJobRunRecordDTO 对象
     * @return 包含更新结果(true/false)的 Result 对象
     */
    @Operation(summary = "更新记录", description = "根据主键更新已有记录")
    @PutMapping("/{id}")
    public Result<Boolean> update(@PathVariable Long id, @RequestBody ScheduledJobRunRecordDTO dto) {
        String userId = SecurityUtils.getCurrentUserId();
        dto.setId(id);
    ...
    /**
     * 根据主键删除 ScheduledJobRunRecord 记录
     *
     * @param id 主键ID
