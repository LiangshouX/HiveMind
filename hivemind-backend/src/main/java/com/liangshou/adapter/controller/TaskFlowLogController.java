...
@RestController
@RequestMapping("/api/task-flow-logs")
@RequiredArgsConstructor
public class TaskFlowLogController {

    private final ITaskFlowLogService service;

    /**
     * 根据主键查询 TaskFlowLog 详情
     *
     * @param id 主键ID
     * @return 包含 TaskFlowLogVO 详情的 Result 对象
     */
    @Operation(summary = "查询详情", description = "根据主键获取单条记录")
    @GetMapping("/{id}")
    public Result<TaskFlowLogVO> getById(@PathVariable Long id) {
        return Result.success(service.getById(id));
    }

    @Operation(summary = "分页查询", description = "分页获取数据列表")
    @GetMapping
    public Result<PageResult<TaskFlowLogVO>> page(
     ...
     * 新增 TaskFlowLog 记录
     *
     * @param dto 包含新增数据的 TaskFlowLogDTO 对象
     * @return 包含保存结果(true/false)的 Result 对象
     */
    @Operation(summary = "新增记录", description = "创建一条新的数据记录")
    @PostMapping
    public Result<Boolean> save(@RequestBody TaskFlowLogDTO dto) {
        return Result.success(service.save(dto));
    }

    /**
     * 根据主键更新 TaskFlowLog 记录
     *
     * @param id  主键ID
     * @param dto 包含更新数据的 TaskFlowLogDTO 对象
     * @return 包含更新结果(true/false)的 Result 对象
     */
    @Operation(summary = "更新记录", description = "根据主键更新已有记录")
    @PutMapping("/{id}")
    public Result<Boolean> update(@PathVariable Long id, @RequestBody TaskFlowLogDTO dto) {
        dto.setId(id);
        return Result.success(service.update(dto));
    }

    /**
     * 根据主键删除 TaskFlowLog 记录
     *
     * @param id 主键ID
