...
CREATE TABLE IF NOT EXISTS `sys_models` (
  ...
  `model_provider_id` VARCHAR(64) NOT NULL COMMENT '模型供应商ID',
  `provider_name` VARCHAR(128) DEFAULT NULL COMMENT 'Provider 显示名称',
  ...
  `is_system_built_in` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否系统内置: 0-否, 1-是',
  `base_url` VARCHAR(255) DEFAULT NULL COMMENT '访问地址',
  `api_key` VARCHAR(512) DEFAULT NULL COMMENT '秘钥, 加密存储',
  `models_json` JSON DEFAULT NULL COMMENT '模型列表 JSON 数组',
  `model_id` VARCHAR(128) DEFAULT NULL COMMENT '请求的模型ID',
  `model_name` VARCHAR(128) DEFAULT NULL COMMENT '模型名称',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_provider` (`user_id`, `model_provider_id`),
  KEY `idx_user_id` (`user_id`),
