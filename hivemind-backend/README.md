# HiveMind 后端项目 (hivemind-backend)

本项目基于 Spring Boot 构建，是 HiveMind 多 Agent 协作平台的后端核心服务。
...
4. **编译构建**:
   ...
   ```
5. **启动项目**:
   在 `hivemind-launcher` 模块下运行 `HiveMindApplication.java`（本项目作为依赖被引入），或者如果本项目配置了启动类，则直接运行本项目的启动类。
