...
export const PIPE = [
  ...
  { key: 'Triage',    dept: '助理',   icon: '🤴', action: '路由' },
  { key: 'Planner', dept: '规划组', icon: '📜', action: '规划' },
  { key: 'Reviewer',   dept: '审查组', icon: '🔍', action: '审查' },
  { key: 'Assigned', dept: '执行组', icon: '📮', action: '执行' },
  { key: 'Doing',    dept: '六部',   icon: '⚙️', action: '执行' },
  { key: 'Review',   dept: '执行组', icon: '🔎', action: '整合' },
  { key: 'Done',     dept: '响应',   icon: '✅', action: '完成' },
] as const;
...
export const DEPT_COLOR: Record<string, string> = {
  '助理': '#e8a040', '规划组': '#a07aff', '审查组': '#6a9eff', '执行组': '#6aef9a',
  '文档组': '#f5c842', '数据组': '#ff9a6a', '搜索组': '#ff5270', '分析组': '#cc4444',
  '工具组': '#44aaff', '管理组': '#9b59b6', '用户': '#ffd700', '响应': '#2ecc8a',
};

export const STATE_LABEL: Record<string, string> = {
  Inbox: '收件', Pending: '待处理', Triage: '消息路由', Planner: '任务规划',
  Reviewer: '方案审查', Assigned: '已执行', Doing: '执行中', Review: '待审查',
...
export function stateLabel(t: Task): string {
  ...
  if (t.state === 'Menxia' && r > 1) return `方案审查（第${r}轮）`;
  if (t.state === 'Zhongshu' && r > 0) return `方案修订（第${r}轮）`;
  return STATE_LABEL[t.state] || t.state;
}

export function isTask(t: Task): boolean {
...
export type TabKey =
  | 'tasks' | 'monitor' | 'officials' | 'models'
  | 'skills' | 'sessions' | 'reports' | 'templates' | 'morning';

export const TAB_DEFS: { key: TabKey; label: string; icon: string }[] = [
  { key: 'tasks',    label: '任务看板', icon: '📜' },
  { key: 'monitor',   label: 'Agent调度', icon: '🏛️' },
  { key: 'officials', label: 'Agent总览', icon: '👔' },
  ...
  { key: 'morning',   label: '最新动态', icon: '🌅' },
];

// ── DEPTS for monitor ──

export const DEPTS = [
  { id: 'triage',    label: '助理',   emoji: '🤴', role: 'Triage Agent',     rank: 'core' },
  { id: 'planner', label: '规划组', emoji: '📜', role: 'Planner Agent',   rank: 'core' },
  { id: 'reviewer',   label: '审查组', emoji: '🔍', role: 'Reviewer Agent',     rank: 'core' },
  { id: 'executor', label: '执行组', emoji: '📮', role: 'Executor Agent',   rank: 'core' },
  { id: 'doc-team',     label: '文档组',   emoji: '📝', role: 'Document Agent', rank: 'team' },
  { id: 'data-team',     label: '数据组',   emoji: '💰', role: 'Data Agent', rank: 'team' },
  { id: 'search-team',   label: '搜索组',   emoji: '⚔️', role: 'Search Agent', rank: 'team' },
  { id: 'analysis-team',   label: '分析组',   emoji: '⚖️', role: 'Analysis Agent', rank: 'team' },
  { id: 'tool-team',   label: '工具组',   emoji: '🔧', role: 'Tool Agent', rank: 'team' },
  { id: 'admin-team',  label: '管理组',   emoji: '👔', role: 'Admin Agent', rank: 'team' },
  { id: 'monitor',  label: '监控', emoji: '🌟', role: 'Monitor Agent',   rank: 'team' },
];
...
export const TEMPLATES: Template[] = [
  {
    ...
    depts: ['数据组', '文档组'], est: '~10分钟', cost: '¥0.5',
    params: [
  ...
  },
  {
    ...
    depts: ['搜索组', '数据组', '文档组'], est: '~60分钟', cost: '¥5',
    params: [
  ...
  },
  {
    ...
    desc: '整合各部今日进展和明日计划，生成站会摘要',
    depts: ['执行组'], est: '~5分钟', cost: '¥0.3',
    params: [
      { key: 'range', label: '整合范围', type: 'select', options: ['今天', '最近24小时', '昨天+今天'], default: '今天' },
    ],
    command: '整合{range}各部工作进展和待办，生成站会摘要',
  },
];
