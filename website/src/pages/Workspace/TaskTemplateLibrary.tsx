...
const TaskTemplateLibrary: React.FC = () => {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      const data = await api.listTaskTemplates();
      setTemplates(data);
    } catch (error) {
  ...
  const execute = async (values: Record<string, string>) => {
    ...
    Modal.confirm({
      title: <span style={{ color: 'var(--td-highlight)', fontSize: '20px' }}>确认执行任务？</span>,
      content: (
        <div style={{
          whiteSpace: 'pre-wrap',
          maxHeight: 200,
      ...
      onOk: async () => {
        try {
          ...
          const result = await api.executeTaskTemplate(formTpl.templateId, params);
          if (result.ok) {
  ...
  const handleSaveTemplate = async (values: any) => {
    try {
      ...
      if (editingTemplate) {
        await api.updateTaskTemplate(editingTemplate.templateId, tplData);
        message.success('模板已更新');
      } else {
        await api.createTaskTemplate(tplData);
            ...
            >
              {formTpl.params.map((p) => (
                <Form.Item
                  ...
                  rules={[{ required: p.required, message: `请填写${p.label}` }]}
                >
                  {p.type === 'textarea' ? (
              ...
              {previewCmd && (
                <div style={{
                  ...
                  <Text strong style={{ color: 'var(--td-primary)', fontSize: '16px' }}>📝 创建任务：</Text>
                  <Paragraph style={{
                    whiteSpace: 'pre-wrap',
                    marginTop: 16,
                    marginBottom: 0,
  ...
  {"key": "format", "label": "格式", "type": "select", "options": ["MD", "PDF"]}
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="depts" label="承办部门（逗号分隔）">
                <Input style={{ background: 'var(--td-input-bg)', color: 'var(--td-text-base)' }} placeholder="数据组, 文档组" />
              </Form.Item>
            </Col>
            <Col span={6}>
      ...
      <Modal
        title={<span style={{ color: 'var(--td-highlight)', fontSize: '20px' }}>✅ 任务已下达</span>}
