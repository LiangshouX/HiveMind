...
const MemorialDetailModal: React.FC<{ task: Task; onClose: () => void; onExport: (t: Task) => void }> = ({ task: t, onClose, onExport }) => {
  const fl = t.flow_log || [];
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      footer={[
        <Button key="export" onClick={() => onExport(t)}>📋 复制奏折</Button>
      ]}
      width={700}
    >
...
const Memorials: React.FC = () => {
  ...
  const exportMemorial = (t: Task) => {
    ...
    navigator.clipboard.writeText(md).then(
      () => message.success('✅ 奏折已复制为 Markdown'),
      () => message.error('复制失败')
    );
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <Title level={4}>御书房 - 奏折</Title>
        <Text type="secondary">管理任务完成后形成的结果，供皇上审阅。</Text>
      </div>
