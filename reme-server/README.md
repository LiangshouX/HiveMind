...
ReMe 的 MCP Service 默认只返回 `response.answer`（人类可读文本），丢弃了 `response.metadata` 中的结构化数据（如文件列表、搜索结果等）。HiveMind 需要这些数据，因此需要对 **pip 安装的 reme 包** 应用补丁：
...
> **此补丁修改的是 pip 安装的 reme 包源码（非 .lib-repo），不影响 ReMe 其他功能。**
> 等待 ReMe 上游合并修复后可移除此补丁。详见 [patches/README.md](patches/README.md)。
